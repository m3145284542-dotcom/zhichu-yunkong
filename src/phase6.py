...
def _dispatch_summary(daily: pd.DataFrame, oracle_daily: pd.DataFrame | None = None) -> dict[str, float]:
    ...
    if oracle_daily is not None:
        merged = daily.merge(oracle_daily[["date", "realized_peak"]], on="date", suffixes=("", "_oracle"), validate="one_to_one")
...
def _selection_key(row: pd.Series) -> tuple[float, float, float, str]:
    return (
        float(row["validation_mean_daily_peak"]),
        float(row["validation_worst_10pct_daily_peak"]),
        float(row["validation_top_quartile_load_MAE"]),
        str(row["candidate"]),
    )


def _write_report(output_dir: Path, report_path: Path, summary: dict[str, Any], test_forecast: pd.DataFrame, test_dispatch: pd.DataFrame) -> None:
    ...
    forecast_cols = ["method", "MAE", "RMSE", "MAPE", "top_quartile_load_MAE", "daily_peak_MAE", "peak_hour_MAE", "peak_hour_hit_within_1h"]
    dispatch_cols = ["method", "mean_daily_peak", "worst_10pct_daily_peak", "max_daily_peak", "mean_peak_reduction", "mean_regret", "p90_regret", "max_regret", "total_throughput"]
    conclusion = (
...
def run(project_root: Path) -> dict[str, Any]:
    ...
    # Strong daily/weekly baseline selection on Validation only.
    baseline_rows: list[dict[str, Any]] = []
    for weekly_weight in BASELINE_WEEKLY_WEIGHTS:
        prediction = (1.0 - weekly_weight) * validation["yesterday_pred"].to_numpy(float) + weekly_weight * validation["last_week_pred"].to_numpy(float)
        _, daily = evaluate_dispatch(validation["target_timestamp"], validation["target"].to_numpy(float), prediction, battery, f"blend_w{weekly_weight:.2f}")
    ...
    baseline_table["selection_rank"] = baseline_table.apply(_selection_key, axis=1).rank(method="first").astype(int)
    baseline_table = baseline_table.sort_values(["validation_mean_daily_peak", "validation_worst_10pct_daily_peak", "validation_top_quartile_load_MAE", "candidate"]).reset_index(drop=True)
    selected_baseline = baseline_table.iloc[0].to_dict()
    baseline_table.to_csv(output_dir / "baseline_selection.csv", index=False)

    # Peak-aware LightGBM selection on Validation only.
    model_rows: list[dict[str, Any]] = []
    fitted: dict[str, tuple[Any, float | None, float, int]] = {}
    for name, quantile, multiplier in PEAK_WEIGHT_CANDIDATES:
        ...
        model, validation_prediction = fit_lgbm(train, validation, ALL_FEATURES, BASE_PARAMETERS, train_sample_weight=weights)
        _, daily = evaluate_dispatch(validation["target_timestamp"], validation["target"].to_numpy(float), validation_prediction, battery, name)
        fm = forecast_metrics(validation["target_timestamp"], validation["target"].to_numpy(float), validation_prediction)
        dm = _dispatch_summary(daily)
        record = {
            "candidate": name,
        ...
        fitted[name] = (model, threshold, multiplier, int(model.best_iteration_))
    model_table = pd.DataFrame(model_rows).sort_values(["validation_mean_daily_peak", "validation_worst_10pct_daily_peak", "validation_top_quartile_load_MAE", "candidate"]).reset_index(drop=True)
    selected_model = model_table.iloc[0].to_dict()
    model_table.to_csv(output_dir / "model_selection.csv", index=False)

    # Freeze selected configuration before any canonical Test loader is called.
    selected_config = {
    ...
    (output_dir / "selected_config.json").write_text(json.dumps(selected_config, ensure_ascii=False, indent=2), encoding="utf-8")

    # Refit the selected peak-aware model using every label available before Test begins.
    ...
    # Test starts here. No parameter choice is permitted after this point.
    ...
    _require(np.allclose(test_features["target_timestamp"].to_numpy(), canonical_test["timestamp"].to_numpy()), "Phase 6 Test target timestamps differ from canonical lineage")
    _require(np.allclose(test_features["target"].to_numpy(float), canonical_test["actual"].to_numpy(float)), "Phase 6 Test actual differs from canonical lineage")
    ...
    forecast_rows = [{"method": name, **forecast_metrics(timestamps, actual, values)} for name, values in forecasts.items()]
    ...
    for daily in daily_parts:
        ...
        dispatch_rows.append({"method": name, **_dispatch_summary(daily, None if name == "Oracle" else oracle_daily)})
    no_battery_daily = pd.DataFrame(
        {
            "method": "No Battery",
            "date": pd.to_datetime(timestamps).dt.date.astype(str),
            "actual": actual,
        }
    ).groupby(["method", "date"], as_index=False)["actual"].max().rename(columns={"actual": "realized_peak"})
    ...
    all_daily = pd.concat([no_battery_daily, *daily_parts], ignore_index=True, sort=False)
    all_daily.to_csv(output_dir / "test_daily_metrics.csv", index=False)
    predictions = pd.DataFrame({"timestamp": timestamps, "actual": actual, **forecasts})
    predictions.to_csv(output_dir / "test_predictions.csv", index=False, date_format="%Y-%m-%d %H:%M:%S")
    dispatches["Phase 6 peak-aware LightGBM"].to_csv(output_dir / "dispatch_phase6_lightgbm.csv", index=False, date_format="%Y-%m-%d %H:%M:%S")
    dispatches["Selected blended persistence"].to_csv(output_dir / "dispatch_selected_blend.csv", index=False, date_format="%Y-%m-%d %H:%M:%S")

    canonical_peak = float(dispatch_table.loc[dispatch_table["method"].eq("Phase 4.5 canonical LightGBM"), "mean_daily_peak"].iloc[0])
    phase6_peak = float(dispatch_table.loc[dispatch_table["method"].eq("Phase 6 peak-aware LightGBM"), "mean_daily_peak"].iloc[0])
    summary = {
        **selected_config,
        "lineage": lineage,
    ...
    _write_report(output_dir, report_path, summary, forecast_table, dispatch_table)
    return summary
