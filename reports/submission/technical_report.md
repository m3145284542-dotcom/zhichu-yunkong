...
<!-- compliance-addition:scenario:start -->
### 办公楼中的使用场景

在办公楼和校园办公区，楼宇能源管理人员需要提前安排储能电池的充放电。设想次日下午出现用电高峰，而电池电量有限：若预测把峰值时刻判断得过早，电池可能提前放电，在真正峰值到来时已没有足够电量。即使全天平均预测误差不大，削峰计划也可能失效。智储云控关注的是预测能否帮助使用者把有限电量留给关键时段，以降低建筑电网侧的最大取电功率。

这一场景用于解释工程需求，不是已部署案例。当前项目通过公开建筑数据完成离线预测、约束调度与结果评价，并用可视化原型支持方案比较；现场接入和运行收益仍待验证。
<!-- compliance-addition:scenario:end -->
...
八建筑测试集提供了一个范围明确的反例：XGBoost 的平均归一化 RMSE 为 0.216751，略低于 LightGBM 的 0.216942，但其平均归一化决策后悔值为 0.117534，高于 LightGBM 的 0.116127（图 1）。两组差距均较小，该结果只说明在本项目的固定样本和调度协议内，预测排序不能完全代替决策排序；它不支持“预测精度不重要”或普适因果结论。

![图 1 预测指标与决策价值不一致的冻结反例](../../outputs/phase13_5/figures_cn/sci_01_forecast_decision_mismatch_cn.png)

**图 1｜预测指标不能单独决定储能决策价值。** 图中比较八建筑测试集等权聚合结果。XGBoost 的归一化 RMSE 略低，但归一化决策后悔值略高，表明两类指标在该实验范围内可能给出不同排序。
...
<!-- compliance-addition:architecture:start -->
## 总体架构设计

智储云控由离线实验计算与结果展示两部分组成。计算端依次执行数据质量控制、因果特征构造、逐建筑模型训练、验证集权重选择及固定约束下的储能调度；展示端读取已经保存的实验结果，供用户比较建筑、预测方法和削峰效果。图 A 展示计算端的数据流与选择边界。验证集用于选模，测试期实际负荷仅在调度计划冻结后用于评价，不进入预测或调度求解。

![图 A 总体技术架构](figures_cn/architecture/doef_architecture_master.png)

**图 A｜总体技术架构。** 历史负荷与日历特征进入预测层，验证集决策指标确定融合权重，测试期使用固定权重和共同电池约束评价实际效果。图中分层区分训练、验证与测试用途；界面展示复用已保存的结果，不触发模型重训。

数据处理由 `src/data` 与 `src/features` 实现，预测训练和验证选择由 `src/phase7` 实现，融合权重与下游调度记录分别保存在 `outputs/phase8` 和 `outputs/phase9`。电池约束与优化器位于 `src/battery`。浏览器界面通过 `gui/server.py` 读取汇总结果，也可使用内嵌数据的离线 HTML。模块间以文件和明确字段衔接，便于独立核对数据来源、权重和结果。
<!-- compliance-addition:architecture:end -->
...
建筑选择表包含 1,578 条建筑序列，其中 296 条为办公建筑候选，82 条通过确定性覆盖与质量规则。最终 8 栋建筑由一栋既定锚点建筑与训练期负荷形态空间中的确定性最远点采样组成。形态特征包括负荷规模、变异系数、峰均比、峰谷差、24 h 与 168 h 自相关、工作日和周末差异、日曲线变异及稳健异常比例。验证集或测试集上的预测、调度成绩均未参与选楼。
...
<!-- compliance-addition:system:start -->
## 系统功能实现

系统提供八建筑巡演与综合结果两个主要视图。巡演视图随回放进度更新建筑和日期，展示无储能、LightGBM 与 DOEF 对应的每日取电峰值曲线，并同步显示所选建筑的电池配置和结果指标。开始巡演后，界面依次回放各建筑；用户可暂停、继续或切换至下一栋，综合结果在八建筑巡演结束后显示。综合结果视图展示逐建筑比较与等权聚合指标，保留负削峰案例。

![图 B 真实系统界面](assets/system_demo.png)

**图 B｜智储云控系统界面。** 截图来自随作品提供的离线 HTML，展示单栋建筑的日峰值回放与运行总览。曲线读取已有实验结果；界面中的播放表示结果回放，不表示正在计算新的调度或控制真实设备。

用户可直接打开随包的离线 HTML，无需联网或安装依赖；也可在源码目录执行 `python gui/server.py`，在浏览器访问 `http://127.0.0.1:8765`。该服务仅依赖 Python 标准库。现有功能支持查看和比较已完成的实验，尚不支持上传任意建筑数据后自动训练，也未接入现场电池控制接口。
<!-- compliance-addition:system:end -->

## 7. 实验结果与分析

<!-- compliance-addition:environment:start -->
### 7.0 实验环境与设置

原实验环境按留存日志列示：最终算法验证记录的处理器标识为 Intel64 Family 6 Model 186 Stepping 2, GenuineIntel，架构为 AMD64，系统标识为 Windows-10-10.0.26200-SP0，Python 为 3.11.9。多建筑阶段记录 pandas 2.3.3。日志未保存内存容量和全部依赖的精确版本，因此不把当前机器配置冒充原实验配置。完整依赖范围由随包 `requirements.txt` 提供，原始环境记录分别见 `outputs/phase9/summary.json` 和 `outputs/phase7/summary.json`。

核心模型采用 CPU 训练，随机种子为 42，线程数为 4。多建筑树模型的最大训练预算为 500 棵树，学习率为 0.03，早停耐心为 50 轮。LightGBM 比较平衡型和小规模正则化两组参数：叶子数分别为 31 和 15，最大深度分别为不限和 6，最小叶样本数分别为 20 和 30，L2 正则项分别为 0 和 0.2；行、列采样比例均为 0.8。逐建筑选定参数及最佳迭代数见 `outputs/phase7/model_selection_config.json`，通用训练实现见 `src/phase7/models.py`。既定锚点建筑的兼容预测按原冻结来源复用，不将统一候选预算描述为对所有历史模型重新训练。

储能优化使用 SciPy 的混合整数线性规划接口，电池容量、效率、SOC 和日末约束沿用前文设置。模型参数与融合权重分别在验证集上确定；测试集不用于追加搜索。包内提供依赖安装、原始数据获取和分阶段执行说明。结果文件核对与从原始数据完整重训是不同的复现层次；本次材料补充仅核对现有结果，未进行完整重训。
<!-- compliance-addition:environment:end -->

### 7.1 预测层结果

表 5 报告冻结测试集的八建筑等权聚合结果。DOEF 的平均归一化 MAE 与 RMSE 在所列方法中最低，分别为 0.137970 和 0.207822。LightGBM 的归一化 MAE 为 0.147338，低于 DayWeek 的 0.183478，但 XGBoost 的归一化 RMSE 以 0.216751 略低于 LightGBM 的 0.216942。不同模型在局部指标上各有优势。MAPE 在低负荷建筑上容易放大，因此不作为跨建筑主结论的依据。
...
相对 LightGBM，DOEF 的平均归一化 MAE 从 0.147338 降至 0.137970，相对改善 6.3585%；平均归一化决策后悔值从 0.116127 降至 0.111929，相对改善 3.6147%（图 2）。建筑级决策后悔值比较为 6 胜、2 平、0 负。以建筑为重采样单位的 10,000 次 bootstrap 给出 DOEF 减 LightGBM 的平均归一化后悔值差为 -0.004198，95% 区间为 [-0.006850, -0.001719]。该区间描述本次 8 栋建筑样本内的不确定性，外部适用性仍需更多建筑验证。

![图 2 DOEF 的测试集预测层与决策层主结果](../../outputs/phase13_5/figures_cn/sci_02_doef_main_results_cn.png)

**图 2｜DOEF 的实验主结果。** 在八建筑测试集等权聚合下，DOEF 相对 LightGBM 同时降低平均归一化 MAE 和平均归一化决策后悔值。图中同时保留平均削峰率与最小削峰率，避免用均值掩盖负值案例。

整段削峰率提供不同于后悔值的工程视角。DOEF 的平均值为 6.4423%，中位数为 5.6291%，最小值为 -1.0170%，最大值为 14.0162%。最小值为负，说明至少一栋建筑在测试期出现反向削峰，即调度后的实际最大负荷高于原始负荷峰值。CatBoost 的最小削峰率为 -245.9433%，反映了预测偏差可能造成的严重调度失效。

对比实验还记录了稳健调度的失效案例。在锚点建筑 Phase 5.7 测试中，验证集选择的稳健调度平均后悔值为 20.4635 kW，高于确定性 LightGBM 调度的 11.1274 kW，因此稳健调度没有进入最终算法。Phase 9 的峰值加权 LightGBM 也没有产生稳定收益，只作为支持性实验记录。

### 7.3 多建筑稳健性

图 3 按冻结选择顺序展示全部 8 栋建筑。DOEF 在 6 栋建筑上的平均归一化决策后悔值低于 LightGBM，在 2 栋上因 DOEF 的冻结 LightGBM 权重为 1.0 而与 LightGBM 持平。该比较包含全部建筑，评价范围为预先固定的办公建筑及一个未来测试月。
...
Phase 9.1 用显式 DOEF 公式、同一组冻结分量预测和 `w_decision` 权重重建测试预测，并与 Phase 8 参考预测逐点比较。8 栋建筑共 5,760 个预测值的最大绝对差为 0.0 kW，容差为 \(10^{-12}\)。同时，Phase 7/8/9/9.1 共 76 个登记科研文件通过 LF 归一化哈希审计。报告直接复用这组预测与调度结果，写作阶段未重新训练、选模或筛选结果。
...
1. 建立预测与储能调度的双层评价方法。在保留 MAE、RMSE、MAPE 的同时，引入实现峰值、削峰率和 Oracle 相对决策后悔值，明确区分预测误差与调度代价。
2. 提出 DOEF，以验证集下游削峰表现选择 DayWeek-LightGBM 的逐建筑融合权重。权重选择直接依据储能削峰任务，预测器和优化器仍分别运行。
3. 建立按时间隔离的多建筑评测流程。建筑依训练期形态选择，训练集、验证集和测试集角色隔离；权重和调度规则冻结后再测试，报告全部 8 栋建筑的结果，并用预测逐点比对和文件哈希检查保证结果可追溯。

LightGBM、周期滞后、通用电池模型和混合整数削峰优化均为已有技术组件，不列为原创算法贡献。

## 9. 工程价值

建筑侧储能容量有限，调度需要把电量留给影响最大需量的时段。将预测接入储能调度，可以直接检验模型对这一控制目标的作用。对于采用日前计划的建筑能源管理系统，这套流程无需改变电池优化器，就能为预测器或预测组合提供与任务直接相关的验证标准。

在八建筑测试中，两分量融合同时改善了预测和决策聚合指标。逐建筑权重、调度约束和结果来源均可查询，便于复现与比较。当前验证采用离线仿真，尚无电价模型、需求电费账单、碳因子、硬件退化成本及现场控制数据，因此未量化成本、能耗与碳排放收益。

<!-- compliance-addition:application:start -->
### 9.1 典型使用流程

楼宇能源管理人员负责核对运行条件和审批计划，储能方案评估人员负责比较预测方案与失败案例。面向办公楼的完整应用流程如下，其中现场数据接入和设备执行属于后续试点设计。

一、 数据接入：目标系统由智能电表或建筑能源管理系统提供历史小时级负荷，核对建筑标识、时间戳、时区、单位与缺失记录。当前实验使用 BDG2 文件，尚未连接实时电表。

二、 负荷预测：DOEF 使用历史负荷与日历特征，按验证阶段确定的逐建筑权重融合 DayWeek 与 LightGBM。当前实现为逐小时提前 24 h 预测，再按目标日组成负荷曲线；现场日前服务还需明确统一数据截止时间、预测可用时间与计划发布时间，并单独验收。

三、 计划生成：预测曲线进入储能优化模块，在电池容量、充放电功率、SOC、效率及日末约束下生成 24 h 充放电计划。输出应包含逐时充放电功率、SOC 轨迹、预计电网取电功率和求解状态，供管理人员审阅。预计结果与实际运行结果分别记录。

四、 执行与回执：当前实验将固定计划作用于实际负荷进行仿真。未来可由 EMS 接收经审批的计划，再经设备适配层交给 PCS/BMS 执行，回传实际功率、SOC、告警与计划执行状态。现场计划必须服从设备侧约束及人工接管；该控制接口尚未实现。

五、 运行评价：离线流程根据实际负荷计算峰值、削峰率和相对 Oracle 的决策后悔值，GUI 展示保存的结果。现场试点应另外评价计划执行偏差、可用率和异常事件；Oracle 仅用于具有完整事后负荷数据的仿真比较，不是现场可调用的预测器。异常记录用于排查与后续独立验证，不回流调整本次冻结测试结果。

本项目当前完成上述流程的离线仿真与可视化验证，尚未接入真实建筑智能电表、储能 PCS/BMS 或现场 EMS，因此不将实验结果表述为实际部署收益。

### 9.2 应用流程与部署条件

当前原型定位为离线仿真决策支持系统。GUI 的实际操作为开始八建筑巡演、查看当前建筑日峰值与电池配置、暂停或切换下一栋、查看八建筑综合比较。界面回放冻结实验，不触发新的预测、调度求解或设备控制。新增建筑需要重新处理数据、训练并独立验证，不能直接沿用已有建筑权重。

试点按离线复现、只读影子运行、人工审批试运行和受控自动运行分阶段推进。影子运行只生成建议与评价，不下发设备指令；只有输入质量、预测时点、物理约束、异常回退及回执链路通过约定验收，才进入下一阶段。现场应读取实测初始 SOC，不能沿用每日重置的仿真假设。具体字段、职责与验收证据见随附工程接入说明。

预期价值是辅助削峰方案筛选和暴露预测失效风险。电费收益、电池寿命、节能与碳减排仍需结合实际费率、退化模型和现场数据另行评价，本作品不提供未经验证的金额或减排承诺。
<!-- compliance-addition:application:end -->

## 10. 局限性、伦理风险与未来工作

数据范围是当前主要限制。实验使用 8 栋预先固定办公建筑和每栋一个未来测试月，只能支持该范围内的多建筑稳健性，不能证明对未见建筑、其他季节或跨年份数据的迁移能力。未来应在预先注册的更多建筑、季节和年份上进行独立外部评价，并保持验证规则不接触新测试集。

预测信息主要来自历史负荷与日历状态。项目未引入更丰富的天气、节假日、占用率或设备状态数据，因此不能判断外部变量是否会改变预测与决策排序。后续可在保持因果可用性的前提下加入这些信息，并分别检查预测层和决策层收益。

电池和调度模型采用固定工程假设，包括按训练期负荷缩放容量、每日 SOC 重置、确定性 24 h 优化和固定效率。它未模拟电池老化、动态电价、光伏不确定性或滚动控制。未来工作应先明确新的实际目标，再扩展为电价、寿命和新能源协同的多目标调度，不能直接把当前削峰率解释为经济收益。

真实建筑在线部署仍需进一步验证。自动调度若依赖偏差较大的负荷预测，可能对不同规模或运行模式的建筑产生不均衡效果；异常数据和模型漂移也可能触发不合理充放电。工程部署应保留人工覆盖、功率与 SOC 硬约束、异常检测、回退基线和持续监测，并记录各建筑的失败案例。当前结果不涉及人员识别或敏感个体数据，但建筑运行数据仍应按数据授权和最小必要原则管理。

## 11. 结论

本作品实现了小时级负荷预测、确定性电池调度和实际峰值评价，并提出决策导向融合方法 DOEF。实验结果表明，预测指标与决策指标在固定八建筑测试中可能给出不同排序。DOEF 根据验证集决策后悔值选择逐建筑 DayWeek-LightGBM 权重，权重冻结后再进入测试评价。相对 LightGBM，DOEF 降低了八建筑平均归一化 MAE 和平均归一化决策后悔值，建筑级决策比较为 6 胜、2 平、0 负；最小削峰率为 -1.02%，仍存在反向削峰。在本次数据、模型和电池配置下，依据决策表现选择融合权重，能够补充传统预测指标，改善储能削峰任务中的模型选择。该结论不延伸到未见建筑、真实在线控制或成本和碳减排收益。
...
本报告核心数值和方法定义对应以下实验文件：`outputs/phase7/selected_buildings.json`、`outputs/phase7/model_selection_config.json`、`outputs/phase7/building_battery_configs.csv`、`outputs/phase8/selected_weights.csv`、`outputs/phase9/final_algorithm.json`、`outputs/phase9/final_benchmark.csv`、`outputs/phase9/aggregate_metrics.csv`、`outputs/phase9/per_building_metrics.csv`、`outputs/phase9_1/final_reporting_summary.json`、`outputs/phase9_1/doef_prediction_invariance.json` 与 `outputs/phase9_1/artifact_hash_audit.json`。写作阶段未训练模型、未调整权重、未重新选择建筑、未运行改变结论的新实验。
