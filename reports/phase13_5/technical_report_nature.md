...
建筑负荷存在日内峰谷和周周期变化。电池储能系统可以在低负荷时段充电、在高负荷时段放电，但调度质量取决于预测误差发生的时间和位置。平均绝对误差（MAE）或均方根误差（RMSE）相近的预测结果，可能形成不同的充放电计划，因此仅按预测指标选模不足以描述模型对削峰任务的实际价值。本项目提出 DOEF v1.0，即面向储能决策的集成负荷预测方法。DOEF 将逐建筑 LightGBM 预测与 DayWeek 周期基线线性融合，在验证集上以相对不可部署 Oracle 的平均日决策后悔值选择权重，随后冻结权重并在测试集上评价。实验使用 Building Data Genome Project 2（BDG2）小时级用电数据，按时间因果切分，并在依据训练期负荷形态和预设数据质量规则选出的 8 栋办公建筑上验证。冻结结果显示，相对 LightGBM，DOEF 的八建筑平均归一化 MAE 从 0.147338 降至 0.137970，平均归一化决策后悔值从 0.116127 降至 0.111929，对应相对改善 6.36% 和 3.61%；建筑级决策后悔值为 6 胜、2 平、0 负。DOEF 的平均整段削峰率为 6.44%，但最小值为 -1.02%，说明该方法仍有失效边界。上述结果支持在冻结仿真协议内以决策价值补充预测指标，不代表真实部署中的成本、能耗或碳排放收益。
...
本项目聚焦 24 h 超前的小时级建筑负荷预测与日内电池削峰。其任务链为：历史负荷及预测时刻可获得的日历信息进入预测器，预测结果驱动 24 h 确定性储能优化，冻结调度再作用于实际负荷，最终由削峰和决策后悔值评价。该任务不涉及在线控制，也未模拟电价、碳排放或建筑总能耗变化。
...
现有建筑负荷预测研究已广泛比较回归和机器学习模型[2,3]，也有工作将负荷或峰值预测用于建筑、园区及电池储能优化[4-8]。更广泛的 predict-then-optimize 和 decision-focused learning 研究则直接讨论预测损失与下游目标的偏离，并发展任务损失训练或决策导向的模型选择方法[9-11]。DOEF 与这些方向共享“预测应由下游用途评价”的问题意识，但实现边界更窄：它不进行端到端训练，也不学习新的优化器，而是在既有 DayWeek 与 LightGBM 预测之间，用验证集储能决策后悔值选择逐建筑固定权重。项目创新因此落在可审计的任务导向融合与冻结评测闭环，而不是 LightGBM、滞后特征或通用削峰优化本身。
...
该设计减少了按测试成绩挑选建筑的风险，但不能被表述为未见建筑迁移。报告中的“多建筑稳健性”仅指 8 栋预先固定办公建筑上的一致性评价。
...
LightGBM 适合处理非线性关系、离散日历状态和多尺度滞后特征，且训练和推理开销较低。本项目使用当前特征时刻的负荷、日历编码、1/2/3/24/48/72/168/336 h 正滞后以及先移位后计算的滚动统计。所有特征均在预测时刻可获得；未使用未来目标、未来天气，也未声称已经加入天气或节假日外部数据。每栋建筑的候选参数和验证集选择结果在冻结配置中记录。LightGBM 是项目采用的基础预测器，不是本项目原创算法。
...
预测误差通过电池计划影响实际峰值，而不是直接等于决策误差。若预测器低估峰值时段，优化器可能预留不足；若高估错误时段，则可能把有限能量配置到非关键时段。两种预测即使具有相近的整体 MAE 或 RMSE，也可能因峰值时序不同而生成不同的充放电轨迹。
...
传统流程通常用验证集 MAE 或 RMSE 选定预测模型，再将其直接输入优化器。DOEF 保留相同的数据隔离原则，但把权重选择指标改为验证集下游储能决策后悔值。由此建立的连接是任务导向的选择连接，而不是可微端到端训练。该做法结构透明、易于复核，但能力也受两个固定预测分量和离散权重网格限制。
...
项目还保留了未进入 DOEF 的负结果。在锚点建筑 Phase 5.7 测试中，验证集选择的稳健调度平均后悔值为 20.4635 kW，高于确定性 LightGBM 调度的 11.1274 kW，因此稳健调度没有进入最终算法。Phase 9 的峰值加权 LightGBM 同样未产生稳定收益，只作为支持性实验记录。
...
DOEF 面向的工程问题是建筑侧储能在有限容量下如何把电量留给真正影响最大需量的时段。其价值不在于单纯追求更低的离线预测误差，而在于用储能调度结果检查预测模型是否服务于既定控制目标。对于采用日 ahead 计划的建筑能源管理系统，该流程可以在不改变电池优化器的条件下，为预测器或预测组合提供任务相关的验证标准。
...
本项目围绕“预测如何服务储能削峰”构建了从小时级负荷预测、确定性电池调度到实际峰值评价的完整链路。冻结结果显示，预测指标与决策指标在固定八建筑测试中可能产生不同排序；DOEF 通过验证集决策后悔值选择逐建筑 DayWeek-LightGBM 权重，并在权重冻结后进行测试评价。相对 LightGBM，DOEF 降低了八建筑平均归一化 MAE 和平均归一化决策后悔值，建筑级决策比较为 6 胜、2 平、0 负，同时保留了 -1.02% 的最小削峰率等不利结果。由此可支持的结论是：在本项目的冻结数据、模型和电池协议内，决策导向选择能够补充传统预测指标，使负荷预测评价更贴近储能削峰任务。该结论不延伸到未见建筑、真实在线控制或成本和碳减排收益。
