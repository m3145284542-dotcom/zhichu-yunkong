...
- 重新导出便携 GUI，HTML 内容不变，修正标题调整后过期的实现哈希。含跨平台哈希回归的 138 项测试通过，材料验证通过全部 858 项。
...
- 修复云端 LF 检出与历史 CRLF 文本哈希的兼容性：仅对明确的 UTF-8 文本类型接受两种换行表示，二进制仍逐字节检查，数值变化仍拒绝。历史哈希清单保持原样；新增回归测试直接读取 Git 对象中的全部 76 项冻结证据。
